...
export async function GET(
  ...
  try {
    ...
    // Validate parameters
    if (!type || !['chat', 'questionnaire'].includes(type)) {
      return NextResponse.json(
        { error: 'Invalid type parameter. Must be either "chat" or "questionnaire"' },
        { status: 400 }
      );
    }
    ...
    try {
      const backendUrl = `http://127.0.0.1:8000/api/health-tracking/history/${params.userId}?type=${type}&page=${page}&limit=${limit}`;
