...
try:
    ...
    def ocr_pdf_to_text(pdf_path, output_txt_path):
        """Fallback OCR function that just writes an error message to the output file."""
        print(f"Error: Could not process PDF {pdf_path}. OCR module not available.")
        with open(output_txt_path, 'w', encoding='utf-8') as f:
            f.write("Error: OCR module not available. Please ensure ocr.py is in the same directory.")
        return False
